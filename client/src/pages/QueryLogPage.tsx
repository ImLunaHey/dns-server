...
export function QueryLogPage() {
  ...
  const { data: initialQueries = [], isLoading: queriesLoading } = useQueries({
    ...
    filters: {
      ...
      minResponseTime: minResponseTime
        ? parseInt(minResponseTime, 10)
        : undefined,
      maxResponseTime: maxResponseTime
        ? parseInt(maxResponseTime, 10)
        : undefined,
    },
  ...
  const handleReplay = (domain: string, type: string) => {
    // Navigate to Tools page with pre-filled values
    navigate({
      to: "/tools",
      search: { domain, type },
    });
  ...
  return (
    <>
      <PageHeader
        title="Query Log"
        description={
          clientIp ? `Viewing queries from ${clientIp}` : "All DNS queries"
        }
      >
        ...
        <Panel className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4 mb-4">
            ...
            <div>
              ...
              <select
                value={
                  blocked === undefined ? "" : blocked ? "blocked" : "allowed"
                }
                onChange={(e) => {
